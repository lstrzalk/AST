...
class Cparser(object):
    ...
    def p_program(self, p):
        ...
        print AST.Program(declarations,fundefs_opt,instructions_opt)
    ...
    def p_instructions_opt(self, p):
        ...
        if p[1] is not None:
            instructions=p[1]
            p[0]=AST.Instructions_opt(instructions)
    ...
    def p_print_instr(self, p):
        """print_instr : PRINT expr_list ';'
                       | PRINT error ';' """
        instr=p[2]
        p[0]=AST.PrintInstruction(instr)

    def p_labeled_instr(self, p):
        """labeled_instr : ID ':' instruction """
        id=p[1]
        instr=p[3]
        p[0]=AST.LabeledInstruction(id,instr)

    def p_assignment(self, p):
        """assignment : ID '=' expression ';' """
        id=p[1]
        expr=p[2]
        p[0]=AST.Assigment(id,expr)
    ...
    def p_repeat_instr(self, p):
        ...
        instruction=p[3]
        condition=p[5]
        p[0]=AST.RepeatInstruction(instruction,condition)
    ...
    def p_expression(self, p):
        ...
        if len(p)==2:
            ...
            exprInside=p[2]
            AST.BracketsExpression(exprInside)
        elif p[2] is '(' and p[1] is not '(':
            name=p[1]
            args=p[3]
            AST.FunctionCalling(name,args)
        else:
            firstExpr=p[1]
            secExpr=p[3]
            operator=p[2]
